...
struct iceberg_mode_v4
  : serde::
      envelope<iceberg_mode_v4, serde::version<0>, serde::compat_version<0>> {
    ss::sstring config;
...
parse_new_format(std::string_view str) {
    using opts_map = absl::flat_hash_map<std::string, std::string>;

    // Phase 1: collect each section's raw options, checking for duplicates and
    // unknown section names before doing any semantic parsing.
    ...
    // Phase 2: parse each collected section's options.
...
void write_nested(iobuf& out, const iceberg_mode& m) {
    ...
    if (
      ...
      && e.headers.value_type == iceberg_mode::header_schema_mode::binary) {
        switch (e.value.mode) {
        case iceberg_mode::schema_mode::binary:
    ...
    // New discriminant 4: the encoded form is the canonicalized config string.
    write(out, wire_canonical_string);
    write(out, iceberg_mode_v4{.config = fmt::format("{}", m)});
}
...
void read_nested(
  iobuf_parser& in, iceberg_mode& m, const std::size_t bytes_left_limit) {
    ...
    case wire_canonical_string: {
        iceberg_mode_v4 v4;
        read_nested(in, v4, bytes_left_limit);
        auto result = parse_iceberg_mode(v4.config);
        if (!result) {
